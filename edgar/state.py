...
def load_state(path: str) -> Dict[str, Set[str]]:
    ...
    path_str = str(path) if hasattr(path, '__fspath__') or hasattr(path, '__str__') else path
    ...
    try:
        ...
        for cik, accessions in data.items():
            ...
            if not isinstance(accessions, list):
                raise ValueError(f"Accessions for CIK {cik} must be list, got {type(accessions)}")
    ...
    try:
        ...
        for cik, accessions in state.items():
            ...
            if not isinstance(accessions, set):
                raise ValueError(f"Accessions for CIK {cik} must be set, got {type(accessions)}")
        ...
        logger.info(f"Saved state for {len(data)} CIKs ({total_accessions} accessions) to {path}")
