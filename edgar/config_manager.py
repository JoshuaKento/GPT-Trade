...
class EdgarConfig:
    ...
    # User agent (required by SEC) - must be set via environment variable or config
    user_agent: str = ""
    ...
    def _validate(self) -> None:
        ...
        # Strict user agent validation for security
        if not self.user_agent or not self.user_agent.strip():
            raise ValueError(
                ...
                "or configure user_agent in config file with format: 'AppName (contact@email.com)'"
            )
        ...
        for key, value in data.items():
            if key == "rate_limit_per_sec":
                ...
                if isinstance(value, str):
                    ...
                    raise ValueError(
                        f"form_types must be list or comma-separated string"
                    )
            else:
                converted[key] = value
...
class ConfigManager:
    ...
    def load_config(
        self, config_path: Optional[Union[str, Path]] = None, use_env_vars: bool = True
    ) -> EdgarConfig:
    ...
    def _load_from_env(self) -> Dict[str, Any]:
        ...
        for env_var, config_key in env_mappings.items():
            ...
            if value is not None:
                ...
                self.logger.debug(
                    f"Using environment variable {env_var} for {config_key}"
                )

        return env_config

    def save_config(self, config: EdgarConfig, config_path: Union[str, Path]) -> None:
