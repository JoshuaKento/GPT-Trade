...
async def monitor_ciks_async(
    ...
    headers = {
        "User-Agent": config.get(
            "user_agent", "GPT-Trade-Agent (your-email@example.com)"
        )
    }
    ...
    # Use the new context manager approach for proper resource management
...
async def monitor_cik_async(
    ...
    Args:
        processor: Async filing processor (with managed session)
    ...
    try:
        ...
        for filing in new_filings:
            task = processor.process_filing_async(
                cik, filing, bucket, prefix, semaphore
            )
        ...
        logger.info(
            f"CIK {cik}: {successful}/{len(new_filings)} filings, {total_docs} documents"
        )
...
Examples:
  ...
  %(prog)s 0000320193 --bucket my-bucket --config config.json --async
    ...
    parser.add_argument("--prefix", help="S3 key prefix (default from config)")
    ...
    parser.add_argument(
        "--manifest", metavar="KEY", help="S3 key for manifest JSON file"
    )
